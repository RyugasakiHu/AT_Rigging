...
class ToolModuleUi(object):
    ...
    def __init__(self,parent,mainUi):
        ...
        pm.button('split Joint',c = self.__splitJoint)
        pm.button('create Box',c = self.__createJointBox)
    ...
    def __splitJoint(self,*args):
        ...
        for i in range(0,num):
            ...
            tempPos = [(parentPos[0] - childPos[0])*(i + 1)/(num + 1) + childPos[0],
                       (parentPos[1] - childPos[1])*(i + 1)/(num + 1) + childPos[1],
                       (parentPos[2] - childPos[2])*(i + 1)/(num + 1) + childPos[2]]
            radius = pm.getAttr(parent[0] + '.radius')
            insertJoint = pm.joint(rad = radius,p = (tempPos[0],tempPos[1],tempPos[2]),n = parent[0] + "_insert_" + str(num - i)) 
            insertJoints.append(insertJoint)
            
            if i == 0:
                pm.parent(child[0],parent[0] + "_insert_" + str(num - i))
            else :    
                pm.parent(parent[0] + "_insert_" + str(num - i + 1),parent[0] + "_insert_" + str(num - i))
                
        #select for next function       
        pm.select(cl = 1) 
        pm.select(parent) 
        for insertJoint in insertJoints:
            pm.select(insertJoint,add = 1)
        pm.select(child,add = 1)        
            
    def __createJointBox(self,*args):
        ...
        #ready to roll:
        for joint in joints:        
            cube = pm.polyCube(n=str(joint) + '_geo0' + str(a),sx=1,sy=1,sz=1)
        ...
        pm.delete(str(joint) + '_geo0' + str(a))    
